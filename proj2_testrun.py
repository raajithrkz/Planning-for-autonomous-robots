...
def animater(explored_node, back_track, path):
    ...
    # Visualize explored nodes around obstacles
    update_frequency = 50  # Update the frame every 50 explored nodes
    for i, state in enumerate(explored_node):
        field[int(y_bounds - state[0]), int(state[1] - 1)] = (200, 255, 200)  # Green color for explored area
        if i % update_frequency == 0:
            out.write(field)
            cv2.imshow('result', field)
            cv2.waitKey(1)

    # Visualize the optimal path with a faster update
    for state in back_track:
        field[int(y_bounds - state[0]), int(state[1] - 1)] = (255, 0, 0)  # Red color for optimal path
        out.write(field)
        cv2.imshow('result', field)
        cv2.waitKey(10)  # Faster update for the optimal path
